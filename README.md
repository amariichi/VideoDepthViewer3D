...
- **Frontend (Vite + Three.js/WebXR)**
  ...
  - **WebXR / Looking Glass:** The RawXR path renders the same reconstructed mesh for generic VR and Looking Glass. The official Looking Glass polyfill is loaded only after explicit activation, so normal monitor startup does not pay its bundle/runtime cost.
...
4. **Optional Looking Glass setup:**
   - Connect the Looking Glass as a desktop display.
   - Install and start Looking Glass Bridge.
   - Use a Chromium-based browser or Firefox, then open a video before selecting
     `Enter Looking Glass`.
...
- **Looking Glass (RawXR):** `Enter Looking Glass` lazily installs the official `@lookingglass/webxr` override, opens its XR presentation, and renders the same calibrated/continuous RawXR mesh across the device views. The nearby state reads `Monitor`, `Starting…`, calibrated `Active`, `Unverified` when an XR preview exists without detected Bridge/device calibration, `Blocked` when pop-up/window-management permission prevented the presentation window, or another actionable failure. `Exit Looking Glass` and session-end both restore monitor rendering.

The Looking Glass polyfill replaces `navigator.xr` for the lifetime of the page.
After Looking Glass is first activated, generic VR is therefore disabled until
the page is reloaded; this avoids sending the VR button through the wrong XR
runtime. Monitor rendering remains available without reloading.
...
fixed so later headset or Looking Glass view motion produces parallax rather than a
head-locked surface.
...
- **フロントエンド (Vite + Three.js/WebXR)**
  ...
  - **WebXR / Looking Glass:** generic VRとLooking Glassの両方で同じRawXR再構成meshを描画します。公式Looking Glass polyfillは明示的な起動時だけ遅延loadされ、通常monitor起動にはbundle/runtime costを加えません。
...
4. **任意のLooking Glass setup:**
   - Looking Glassをdesktop displayとして接続します。
   - Looking Glass Bridgeをinstallして起動します。
   - Chromium系browserまたはFirefoxで動画を開いてから、
     `Enter Looking Glass`を選択します。
...
- **Looking Glass (RawXR):** `Enter Looking Glass`で公式`@lookingglass/webxr` overrideを遅延installし、同じ校正済み連続RawXR meshをdeviceの各viewへ描画します。隣接statusは`Monitor`、`Starting…`、Bridge/device校正を検出した`Active`、XR previewはあるが校正未検出の`Unverified`、popup/window-management permissionでpresentation windowを開けない`Blocked`、または対処可能なfailureを示します。`Exit Looking Glass`とsession終了はいずれもmonitor表示へ戻ります。

Looking Glass polyfillはpage lifetime中の`navigator.xr`を置き換えます。一度Looking Glassを
起動した後は、誤ったXR runtimeへVR buttonを送らないようgeneric VRをpage reloadまで
無効化します。monitor表示はreloadせず引き続き利用できます。
