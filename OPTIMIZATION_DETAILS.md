...
at 30 FPS when the source FPS and hardware permit it. RGB video remains at the
browser's native playback quality; automatic modes spend depth resolution,
transport precision, and mesh density instead.
...
深度表示30 FPSを目標にします。ブラウザで再生するRGB動画の画質は維持し、自動モードは
深度解像度、転送精度、メッシュ密度を調整します。
